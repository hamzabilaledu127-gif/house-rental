...
<td  bgcolor="	#ffccff "valign="top"height="150px" width="325px" margin-top="0px" border="0px" align-top="0px"><!--center -->
	  ...
	  <div id="pagetitle" ><!-- InstanceBeginEditable name="pagetitle" --><b> Administrator Page To send Response For Landlord</b><!-- InstanceEndEditable --></div>
     	</br>
...
$con = mysqli_connect('localhost','root', '','house');//Records per page?>
 ...
 <FORM ACTION="activationresponse.php" METHOD="POST" enctype="multipart/form-data">
...
<tr>
<td>CustomerUserName :</td><td><input name="username" id ="username"  type="text" size"20"  required placeholder="Enter username name"></input></td>
 <tr>
      <td >enter your message </td>
      <td bgcolor=""><label>
              <textarea colspan="2" cols="22" rows="2" name="message" id="message" value=" " title="Enter last name here" required "></textarea><br>
      </label></td>
         </tr>
          <tr>
      <td a>Date</td>
...
<input type="submit" value="Send" name="add"/> <input type="reset" value="Reset"/>
</td></tr>
...
<?php
 ...
 $username=$_POST['username'];
   $date=$_POST['date'];
         //$code=$_POST['message'];
  $sql2="select * from leavehouse where username='{$username}' ";
  $re2=mysqli_query($con,$sql2);
  while($row=mysqli_fetch_array($re2)){
  $email=$row['email'];
  $username=$row['username'];
  $housenumber=$row['housenumber'];;
$sql="insert into activationresponse (username,email,housenumber,message,date)
  values('$_POST[username]','$email','$housenumber','$_POST[message]','$_POST[date]')";  
     $result=mysqli_query($con,$sql); 
        $result = mysqli_query($con,("update register set status='' WHERE housenumber='{$housenumber}' And status='reserved' ")) or die(mysqli_error());  
        
if(!$result){
    print "<img src='IMG/error.png' align='center' width='25px' height='25px'><font face='arial' color='red' size='2'> Invalid Entry!!</font>";
   echo' <meta content="5;activationresponse.php" http-equiv="refresh" />'; 
}
